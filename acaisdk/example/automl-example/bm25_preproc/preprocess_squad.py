...
def preprocess(path, output_path, newPrepocess=True):
    if newPrepocess:
        # Load dataset
        ...
        saveData(train, os.path.join(output_path, 'train-preprocessed.json'))
        saveData(dev, os.path.join(output_path, 'dev-preprocessed.json'))
        return train, dev
    else:
        # Load
